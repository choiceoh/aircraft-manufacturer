...
(function (root) {
  ...
  const B = {
     ...
     */
    /**
     * 운임 눈금 — 이 계층의 난이도가 걸린 한 숫자다.
     *
     * **화물이 들어오면서 0.15 → 0.125 로 내렸다.** 화물은 매출의 9% 를 새로 얹는데,
     * 0.15 는 화물이 없던 시절에 "20년 자기자본 1.6배"를 노리고 잡은 값이라 그대로 두면
     * 그만큼 통째로 쉬워진다 — 실제로 자동조종 120판에서 배수 1.62 → 2.01, 파산
     * 0.5 → 0/12, 중앙 등급이 B 에서 A 로 올라갔다. 여객 쪽에서 그만큼 돌려주면
     * 난이도 곡선은 제자리에 두고 **모양만** 바뀐다: 광동체·장거리가 상대적으로 좋아진다.
     *
     * 재보정 결과 배수 1.64 · 등급 S9·A22·B47·C21·D18·F3 로 화물 이전(1.62 ·
     * S9·A24·B48·C19·D15·F5)과 사실상 같은 자리에 섰다.
     */
    FARE_SCALE: 0.125,
